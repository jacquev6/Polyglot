...
g++ -fPIC -shared $(pkg-config python2 --cflags) wrappermodule.cpp -lboost_python-py27 -L. -lguest -o wrapper.so

LD_LIBRARY_PATH=.:$LD_LIBRARY_PATH python2 host.py

g++ -fPIC -shared $(pkg-config python3 --cflags) wrappermodule.cpp -lboost_python-py36 -L. -lguest -o wrapper.so
