...
g++ -fPIC -shared $(pkg-config python2 --cflags) wrapper.cpp -L.. -lguest_cpp -o wrapper.so

LD_LIBRARY_PATH=..:$LD_LIBRARY_PATH PYTHONPATH=.:$PYTHONPATH python2 ../host.py

g++ -fPIC -shared $(pkg-config python3 --cflags) wrapper.cpp -L.. -lguest_cpp -o wrapper.so
